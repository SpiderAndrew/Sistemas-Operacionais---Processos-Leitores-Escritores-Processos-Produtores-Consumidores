...
void escrever_mensagem(int id) {
    ...
    for(int i = 0; msg[i] != '\0'; i++) {
        chat_buffer[i] = msg[i];
        usleep(50000); // Atraso para causar a leitura suja
    }
...
int main() {
    srand(time(NULL));
    int num_leitores = 5, num_escritores = 2; // Você pode pedir via scanf também
    pthread_t leitores[num_leitores], escritores[num_escritores];

    printf("=== V1: SEM PRIORIDADE (Permite Leitura Suja) ===\n\n");
